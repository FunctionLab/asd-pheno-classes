...
def compute_odds_ratios():
    ...
    dnvs_pro['lof'] = dnvs_pro['Consequence'].apply(
        lambda x: 1 if x in consequences_lof else 0)
    dnvs_sibs['lof'] = dnvs_sibs['Consequence'].apply(
        lambda x: 1 if x in consequences_lof else 0)
    ...
    for class_id, class_count in zip([0,1,2,3], [num_class0, num_class1, num_class2, num_class3]):
        ...
        lof_gene_vars_for_class = gene_vars_for_class[
            gene_vars_for_class['lof_final_consequence'] == 1]
    ...
    for i in range(len(odds_ratios_for_plotting[0])):
        ...
        ax2.bar(x_values, y_values, width=bar_width, 
                label=f'Value {i+1}', color=colors[i])
    ...
    dnvs_pro['synonymous'] = dnvs_pro['Consequence'].apply(
        lambda x: 1 if x in consequences_benign else 0)
    dnvs_sibs['synonymous'] = dnvs_sibs['Consequence'].apply(
        lambda x: 1 if x in consequences_benign else 0)
    dnvs_pro['lof'] = dnvs_pro['Consequence'].apply(
        lambda x: 1 if x in consequences_lof else 0)
    dnvs_sibs['lof'] = dnvs_sibs['Consequence'].apply(
    ...
    for class_id, class_count in zip([0,1,2,3], [num_class0, num_class1, num_class2, num_class3]):
        ...
        # Syn
        syn_gene_vars_for_class = gene_vars_for_class[
            gene_vars_for_class['syn_final_consequence'] == 1]
    ...
    for i in range(len(odds_ratios_for_plotting[0])):
        x_values = [x + i * bar_width+0.075 for x in x_positions]
        y_values = [item[i] for item in odds_ratios_for_plotting]
        ax1.bar(x_values, y_values, width=bar_width, 
                label=f'Value {i+1}', color=colors[i])
    ...
    plt.tight_layout()
    plt.savefig(
        'figures/WES_odds_ratios_figure.png', 
        bbox_inches='tight', 
        dpi=600
        )
    plt.close()
