...
def make_gene_trend_figure(fdr=0.05):
    ...
    for gene_set, trend, category in zip(gene_set_names, trends, cell_type_categories):
        ...
        if np.max([class0_pval, class1_pval, class2_pval, 
                   class3_pval, sibs_pval, all_pros_pval]) < -np.log10(fdr):
            continue
        ...
        class3_df = pd.DataFrame({'variable': gene_set, 'value': class3_pval, 
                                  ...
                                  'trend': trend}, index=[0])
        all_pros_df = pd.DataFrame({'variable': gene_set, 'value': all_pros_pval, 
                                    'Fold Enrichment': all_pros_fe, 'cluster': -1, 
                                    'trend': trend}, index=[0])
        sibs_df = pd.DataFrame({'variable': gene_set, 'value': sibs_pval, 
    ...
    for _, row in validation_subset.iterrows():
        if row['value'] < -np.log10(fdr):
            plt.scatter(row['Cluster'], row['variable'], 
                        s=row['Fold Enrichment']*210, c='white', linewidth=2.5, 
                        edgecolors=row['color'], alpha=0.9)
        else:
            plt.scatter(row['Cluster'], row['variable'], 
                        s=row['Fold Enrichment']*230, c=row['color'])

    for i in range(2, 16, 3): # add legend sizes
        plt.scatter([], [], s=(i)*230, c='dimgray', label=str(i+1))
    plt.legend(scatterpoints=1, labelspacing=2.8, title='Fold Enrichment', 
